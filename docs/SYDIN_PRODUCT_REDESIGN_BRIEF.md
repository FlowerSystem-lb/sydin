...
22. ✅ Report builder. ✅ date range, applied to every sales/purchase/payment report at once (14 Sep) — Sayed's pick, over a full custom-columns builder, when asked which was worth building first. ✅ on-screen preview (20 Sep): a money report opens as a table in a sheet first, PDF and CSV one tap from there — the phone no longer downloads a file blind. ✅ columns and sort (24 Sep): chips over the preview hide/show any column but the first, any heading sorts (amounts biggest first, names and dates A–Z, blanks last), and the CSV and PDF carry the same view. ✅ saved reports (24 Sep): name a view with its dates, it sits at the top of Reports and reopens in one tap — kept in the browser per account (a table would be a schema change; the page says "On this device"). ✗ grouping: the money reports are already the groupings (by month, customer, category, supplier, method); a regroup control over pre-aggregated rows would have to recompute totals, which this layer deliberately never does.
...
61. ◐ Bug hunt — 20 Sep sweep: every internal link from the main pages fetched (38, all respond); console clean on Overview / Inventory / POs / Sales; missing-item and missing-invoice pages now use the shared empty state (were a hand-rolled card and a red line). Phone overflow sweep clean. 24 Sep — all 18 sidebar pages opened on the live site with real data: no error banners, no horizontal overflow, and the Supabase request log shows no failed request from the sweep. The one real noise found (Scanner laptop pairing lookup returning 406 whenever no phone is paired) now uses maybeSingle; the Scanner migration probes still log a harmless 400 by design. Ongoing as surfaces change.
